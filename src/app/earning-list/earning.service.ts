...
export class EarningService {
  ...
  getTotalServices(data?): Observable<any> {
    let url
    if (data) {
      url = `/earnings/services?startDate=${data.startDate}&endDate=${data.endDate}&barberId=${data.barberId}`
    } else {
      url = '/earnings/services?startDate=all&&endDate=all&barberId=all'
    }
  ...
  getTotalProducts(data?): Observable<any> {
    let url
    if (data) {
      url = `/earnings/products?startDate=${data.startDate}&endDate=${data.endDate}&barberId=${data.barberId}`
    } else {
      url = '/earnings/products?startDate=all&&endDate=all&barberId=all'
    }
